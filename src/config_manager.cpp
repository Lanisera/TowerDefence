	...
	cJSON_ArrayForEach(json_wave, json_root)
	{
		...
		if (json_wave_interval && json_wave_interval->type == cJSON_Array)
		{
			...
			cJSON_ArrayForEach(json_spawn_event, json_wave_spawn_list)
			{
				...
				{
					const std::string enemy = json_spawn_event_enemy->valuestring;
					if (enemy == "Slim")
						spawn_event.type = EnemyType::Slim;
					else if (enemy == "KingSlim")
						spawn_event.type = EnemyType::KingSlim;
					else if (enemy == "Skeleton")
						spawn_event.type = EnemyType::Skeleton;
					else if (enemy == "Goblin")
						spawn_event.type = EnemyType::Goblin;
					else if (enemy == "GoblinPriest")
						spawn_event.type = EnemyType::GoblinPriest;
				}
			}
...
void ConfigManager::parse_basic_template(BasicTemplate& tpl, cJSON* json_root)
{
	...
	cJSON* json_window_tile = cJSON_GetObjectItem(json_root, "window_title");
	...
	if (json_window_tile && json_window_tile->type == cJSON_String)
		tpl.window_title = json_window_tile->valuestring;
