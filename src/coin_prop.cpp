...
CoinProp::CoinProp()
{
	...
	timer_jump.set_wait_time(jumping_interval);
	...
	timer_disappear.set_wait_time(disappear_interval);
...
void CoinProp::on_render(SDL_Renderer* renderer)
{
	...
	static SDL_Texture* tex_coin = 
		ResourcesManager::instance()->get_texture_pool().find(ResID::Tex_Coin)->second;
